      ...
      <header className="h-16 flex items-center border-b border-zinc-200/60 dark:border-zinc-800/60 bg-white/70 dark:bg-zinc-900/70 backdrop-blur-xl sticky top-0 z-20 transition-colors">
        <div className="max-w-4xl mx-auto w-full px-6 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="font-bold text-sm tracking-tight text-zinc-800 dark:text-zinc-100">
                {chatId ? `Chat • ${chatId.slice(0, 8)}` : 'NextGen AI'}
              </h1>
          ...
          {messages.length === 0 ? (
            <div className="flex-1 flex flex-col items-center justify-center text-center space-y-8 py-20">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 w-full max-w-lg pt-4">
                {['¿Cómo está el clima en Madrid?', 'Explícame qué es una PWA', 'Dame una receta de pasta', 'Escribe un script en Python'].map((prompt) => (
                  <button
                    key={prompt}
              ...
              {messages.map((message) => (
                ...
                >
                  <div className={`flex flex-col max-w-[85%] ${message.role === 'user' ? 'items-end' : 'items-start'}`}>
                    <div className={`relative px-5 py-3.5 rounded-2xl ${message.role === 'user' ? 'bg-linear-to-br from-blue-600 to-indigo-600 text-white shadow-lg shadow-blue-500/20' : 'bg-white dark:bg-zinc-900 border border-zinc-200/80 dark:border-zinc-800/80 text-zinc-800 dark:text-zinc-200 shadow-sm'}`}>
                      <div className="space-y-4">
                        {message.parts.map((part, i) => (
                          <div key={i}>
                            ...
                            {isToolUIPart(part) && (
                              <div className="mt-4">
                                {part.type === 'tool-weather' && part.state === 'output-available' && <WeatherCard data={part.output} />}
                                {(part.state === 'input-streaming' || part.state === 'input-available') && (
                                  <div className="flex items-center gap-3 px-4 py-3 bg-zinc-50 dark:bg-zinc-800/50 rounded-xl border border-dashed border-zinc-200 dark:border-zinc-700">
                                    ...
                                    <span className="text-xs font-bold text-zinc-500 uppercase tracking-widest">Checking tool...</span>
                                  </div>
                                )}
                              </div>
                            )}
        ...
        <form onSubmit={handleSubmit} className="max-w-3xl mx-auto relative group">
          ...
          <div className="relative flex items-center bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl shadow-2xl transition-all group-focus-within:border-blue-500/50">
            ...
            <div className="pr-2 flex items-center gap-2">
              <button type="button" className="p-2.5 text-zinc-400 hover:text-zinc-600 dark:hover:text-zinc-200 transition-colors" title="Subir archivo">
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15.172 7l-6.586 6.586a2 2 0 102.828 2.828l6.414-6.586a4 4 0 00-5.656-5.656l-6.415 6.585a6 6 0 108.486 8.486L20.5 13" />
                </svg>
              </button>
              <button type="submit" disabled={isLoading || !input.trim()} className="bg-zinc-900 dark:bg-white text-white dark:text-zinc-950 p-2.5 rounded-xl disabled:opacity-30 disabled:cursor-not-allowed hover:scale-105 active:scale-95 transition-all shadow-lg">
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <p className="mt-2.5 text-center text-[10px] text-zinc-400 dark:text-zinc-500 font-medium">NextGen AI puede cometer errores. Considera verificar la información importante.</p>
        </form>
      </div>
    </div>
  );
